...
class reprojectTo3D():
    ...
    def setStereoParameter(self,minDisparity:int=0, numDisparity:int=96, blockSize:int=16, windowSize:int=3, p1:int=8, p2:int=32, 
    ...
    def makeStereoRectifyImg(self, img1, img2, img1Intrinsic:dict, img2Intrinsic:dict, extrinsic:dict, mtxKey:str="mtx", distKey:str="dist"):
        ...
        #img1 rectify
        map1, map2 = cv2.initUndistortRectifyMap(cameraMatrix=mat1,distCoeffs=dist1,R=R1,newCameraMatrix=P1, size=img_size, m1type=cv2.CV_32FC1)
        reImg1 = cv2.remap(src=img1, map1=map1, map2=map2,interpolation=cv2.INTER_NEAREST)
        
        #img2 rectify
        map1, map2 = cv2.initUndistortRectifyMap(cameraMatrix=mat2,distCoeffs=dist2,R=R2,newCameraMatrix=P2, size=img_size, m1type=cv2.CV_32FC1)
